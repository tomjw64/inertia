...
impl HeuristicBoard {
  ...
  pub fn can_prune(
    ...
    {
      if base_heuristic + augment_amount > depth_allowance
        && (actor_squares_bitboard & unoccupied_augment_bitboard).is_zero()
      {
        return true;
      }
      // We assume:
      //  - a) that the bitboards are sorted in order of ascending augment
      //    amount
      //  - b) that if the bitboard with the lowest sufficient augment amount
      //    won't work that nothing with a higher augment amount will
      // if base_heuristic + augment_amount > depth_allowance {
      //   return (actor_squares_bitboard & unoccupied_augment_bitboard)
      //     .is_zero();
      // }
    }
  ...
  pub fn from_block_board(board: &BlockBoard, goal: Square) -> Self {
    let goal_index = goal.0;
